...
import { useStatisticsCardViewModel } from "./statistics-card.viewmodel";
import { Skeleton } from "@/userinterface/components/ui/skeleton";

export function StatisticsCard() {
  const { currentStreak, highestStreak, isLoading, loadStreakData } =
    useStatisticsCardViewModel();
      ...
      <Card className="@container/card highlight-card">
        <CardHeader>
          ...
          <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl flex items-center gap-2">
            {isLoading ? (
              ...
              <>
                {currentStreak} <IconFlame className="size-7 text-orange-500" />
              </>
            )}
          </CardTitle>
          <CardAction>
            <Badge variant="outline">
              <IconTrendingUp />
              {currentStreak === highestStreak && currentStreak > 0
                ? "Record!"
                : `Max: ${highestStreak}`}
            </Badge>
          </CardAction>
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          ...
          <div className="text-muted-foreground">
            {currentStreak > 0
              ? "Continuez comme ça!"
              : "Remplissez votre journal aujourd'hui!"}
          </div>
        </CardFooter>
      </Card>
      <Card className="@container/card">
